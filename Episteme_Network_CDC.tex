...
\usetikzlibrary{positioning, arrows.meta, shapes, shapes.geometric, calc, mindmap, trees, shadows, fit, babel}
...
% ANNEXES VISUELLES
% ============================================================================
\chapter{Annexes Visuelles}

\section{Interface Principale (Vue Graphe)}
La vue principale permet d'explorer le réseau d'influence. Les nœuds sont colorés par communauté (Louvain) et dimensionnés selon leur importance (PageRank).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{episteme_graph_v5.png}
    \caption{Visualisation du réseau d'influence scientifique (v5)}
    \label{fig:graph_v5}
\end{figure}

\newpage

\section{Rapport d'Analyse (Small World)}
Le rapport généré automatiquement par \texttt{small\_world\_analysis.py} confirme les propriétés topologiques du réseau : un fort clustering associé à une distance moyenne faible (effet "Petit Monde").

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth, trim={0 0 0 0}, clip]{small_world_report.png}
    \caption{Tableau de bord de l'analyse topologique (Design "Swiss Grid")}
    \label{fig:sw_report}
\end{figure}

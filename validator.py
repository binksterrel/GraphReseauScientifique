...
class WikidataValidator:
    ...
    def _cache_set(self, query_hash: str, data: dict) -> None:
        path = self._get_cache_path(query_hash)
    ...
    def _sparql_query(self, query: str) -> Optional[dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Wikidata error: {response.status_code}")
                return None

        except requests.RequestException as e:
            logger.warning(f"Wikidata request failed: {e}")
            return None
    ...
    def validate_relation(self, source_name: str, target_name: str) -> Dict:
        ...
        """
        logger.debug(f"Validating: {source_name} <- {target_name}")
...
def validate_graph_sample(gexf_path: str, sample_size: int = 20) -> None:
    ...
    logger.info(f"Validating {len(edges)} relations against Wikidata...")
    ...
    for source, target in edges:
        ...
        if result["validated"]:
            validated_count += 1
            logger.info(f"VALID: {source} <- {target} (confidence: {result['confidence']:.2f})")
        else:
            logger.info(f"UNVALIDATED: {source} <- {target} (confidence: {result['confidence']:.2f})")

    logger.info(f"Validation Summary: {validated_count}/{len(edges)} validated")
